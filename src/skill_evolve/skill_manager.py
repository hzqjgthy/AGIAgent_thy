...
class SkillManager:
    ...
    def __init__(self, root_dir: Optional[str] = None, config_file: str = "config/config.txt"):
        ...
        if self.api_key and self.model:
            ...
            should_use_anthropic_sdk = is_real_anthropic or is_anthropic_compatible
            
            # Anthropic原生与Anthropic兼容端点统一走Anthropic SDK
            if should_use_anthropic_sdk and ANTHROPIC_AVAILABLE:
                try:
                    ...
                    logging.getLogger('skill_manager').warning(
                        f"Failed to initialize Anthropic client: {e}, falling back to OpenAI-compatible client"
                    )
                    self.llm_client = None
                    self.is_claude = False
            ...
            if not self.llm_client and OPENAI_AVAILABLE:
                try:
                    ...
                    logging.getLogger('skill_manager').warning(f"Failed to initialize OpenAI-compatible client: {e}")
