...
class ToolExecutor:
    def __init__(self, api_key: Optional[str] = None,
        ...
        # Load tool result logging switch from config/config.txt
        self.log_tool_results = get_log_tool_results()
...
"""
    ...
    def execute_subtask(self, prompt: str, prompts_file: str = "", 
        ...
        try:
            ...
            if tool_calls:
                ...
                # Optionally append complete tool execution results to manager.log
                if self.log_tool_results:
                    print_debug(
                        f"\n--- Tool Execution Results (round {round_counter}) ---\n"
                        f"{tool_results_message}\n"
                        f"--- End Tool Execution Results ---"
                    )
    ...
    def _format_generic_field(self, key: str, value: Any) -> str:
        ...
        if key == "admissible_commands":
            # Keep the full candidate-action list visible to the model.
