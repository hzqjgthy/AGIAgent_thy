...
def get_log_tool_results(config_file: str = "config/config.txt") -> bool:
    """
    Get tool result logging configuration from configuration file.
    
    Args:
        config_file: Path to the configuration file
        
    Returns:
        Boolean indicating whether to append tool execution results to manager.log (default: False)
    """
    config = load_config(config_file)
    log_tool_results_str = config.get('log_tool_results', 'False').lower()
    
    # Convert string to boolean
    if log_tool_results_str in ('true', '1', 'yes', 'on'):
        return True
    elif log_tool_results_str in ('false', '0', 'no', 'off'):
        return False
    else:
        print(f"Warning: Invalid log_tool_results value '{log_tool_results_str}' in config file, using default False")
        return False
